...
class MarketOrder extends Component<Props, State> {
    ...
    sliderChange = (step: number) => {
        if (this.state.selectedTab === 'buy') {
            // per / 100 * total =   get
            const total = Number(((step / 100) * this.props.bWallet.available!).toFixedFloor(this.props.mainPair?.marketPrecision!));
            ...
            const amt = Number(((step / 100) * this.props.mWallet.available!).toFixedFloor(this.props.mainPair?.basePrecision!));
    ...
    createOrder = async () => {
        ...
        //     }
        // }
        const res = await OrderService.getInstance().createOrder(modal);
                            ...
                            <div className="mb-3">
                                ...
                                <div className="md:flex justify-between mb-1">
                                    ...
                                    <div className="text-xs text-mute md:block hidden">
                                        ...
                                        {this.props.isLogin ? (this.props.bWallet?.balance ?? '-') : '-'}<span className='text-white'>{this.props.mainPair?.marketCurrency?.symbol}</span>
                                    </div>
                                    <div className="text-xs text-mute md:hidden flex justify-end">
                                        Balance:
                                        <span className='text-white mx-1'>
