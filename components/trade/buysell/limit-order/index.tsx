...
class LimitOrder extends Component<Props, State> {
    ...
    sliderChange = (step: number) => {
        if (this.state.selectedTab === 'buy') {
            ...
            const amt = Number(((step / 100) * this.props.bWallet.available!).toFixedFloor(this.props.mainPair?.marketPrecision!));
    ...
    createOrder = async () => {
        ...
        if(res?.OrderId > 0){
            CustomNotification.show('order created', CNotificationType.Success)
